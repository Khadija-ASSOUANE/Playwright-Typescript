/**
 * @file        8_DELETE_API_Request.spec.ts
 * @chapter     API Testing and Mocking - DELETE API Request
 * @description Demonstrates a full DELETE API flow on OrangeHRM: POST to auth
 *              for a token, POST to create a user, then DELETE that user.
 *              Tags demonstrate CI/CD pipeline integration annotations.
 *
...
test('[10,11] Create DELETE API Request using playwright & typescript', { tag: ['@PlaywrightWithJenkins', '@PlaywrightWithAzureDevOpsPipeline','@PlaywrightWithGitHubActions'] }, async ({ request }) => {
    ...
    const postAPIRequest = await getPOSTAPIRequestBody(firstName, lastName, totalPrice,
        true, "breakfast", "2027-10-21", "2027-10-27");
    ...
    expect(jsonPOSTAPIResponse.booking.bookingdates.checkin).toBe('2027-10-21');
    expect(jsonPOSTAPIResponse.booking.bookingdates.checkout).toBe('2027-10-27');
